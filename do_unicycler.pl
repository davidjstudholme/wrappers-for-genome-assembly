...
while (defined(my $file = readdir(DIR))) {
	...
	my $reads_file_1 = $file;
	my $reads_file_2 = "${1}_${2}_${3}_R2_${4}.fastq.gz";

	my $trimmed_reads_file_1 = "${1}_${2}_${3}_R1_${4}_val_1.fq.gz";
	my $trimmed_reads_file_2 = "${1}_${2}_${3}_R2_${4}_val_2.fq.gz";

	### Trim the Illumina reads
	if ( -s $trimmed_reads_file_1 and -s $trimmed_reads_file_2) {
	    warn "$trimmed_reads_file_1 and $trimmed_reads_file_2 already exist\n";
	    #system("ls -lh $trimmed_reads_file_1  $trimmed_reads_file_2");
	} else {
	    warn "Will perform trim_galore on $reads_file_1 and $reads_file_2\n";
	    my $tg_command = "trim_galore -q 30 --paired $reads_file_1 $reads_file_2";
	    warn "$tg_command\n";
	    system($tg_command);
	}
	...
	if (-s $nanopore_file1 > 1000  and -s $nanopore_file2 > 1000) {
	    ...
	    warn "Missing files or empty files: $nanopore_file1 and $nanopore_file2";
	}
	...
	### Perform Unicycler hybrid assembly
	my $unicycler_path = "/data/djs217/Unicycler-0.5.1/unicycler-runner.py";
	my $unicycler_out = "${2}.runs1-and-2.unicycler";
	
	#if (-d "$unicycler_out/assembly.fasta") {
	if (-d $unicycler_out and -s "$unicycler_out/assembly.fasta") {
	    
	    warn "$unicycler_out already exists\n";
	} else {
	    my $unicycler_command = "$unicycler_path -1 $trimmed_reads_file_1 -2 $trimmed_reads_file_2 -l $trimmed_nanopore_file -o $unicycler_out";
	    warn "\nMake sure that you have activated conda environment unicyler_env\n\n";
	    warn "$unicycler_command\n";
	    system($unicycler_command);
	}
